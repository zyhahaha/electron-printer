...
// 核心绘制方法
function printJson2CanvasBase64(drawingData, canvasWidth, imageData) {
  ...
  drawingData.forEach((item, index) => {
      ...
      drawList.push(() => {
        ...
        if (item.wrapText) {
          ...
          line = wrapTextFn(
            ctx,
            item.content,
            calculateX(ctx, item, canvas),
            dragStartY.value,
            maxWidth,
            parseInt(item.fontSize, 10) + 10
          )
        } else {
    ...
    //             // 10个偏移，为了让图片向下调整一点，防止被切掉
    //             ctx.drawImage(barcodeCanvas, canvas.width - barcodeCanvas.width, dragStartY.value - barcodeCanvas.height + 10);
    //         } else {
    ...
    if (item.type === 'image') {
      ...
      drawList.push(() => {
        ...
        ctx.drawImage(
          imageData,
          canvas.width / 2 - item.width / 2,
          dragStartY.value - item.height,
          item.width,
          item.height
        )
