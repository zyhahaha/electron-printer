<template>
  ...
  <div class="actions">
    ...
    <div class="action">
      <a target="_blank" rel="noreferrer" @click="openPrintHistory">Print History</a>
    </div>
  </div>

  <p v-for="(printItem, key) in printHistoryList" :key="key">{{ printItem }}</p>
  <!-- <table-lite
...
const printHistoryList = ref([])
const openPrintHistory = async () => {
  printHistoryList.value = JSON.parse(localStorage.getItem("printHistory") || "[]")
};
